...
#include <clusters/RandomNumberGenerator/MetadataProvider.h>
	...
	namespace DataModel
	{
		namespace detail
		{
											       ...
											       CommandId command)
			{
				...
				if constexpr (sizeof...(TClusterIds) == 0 ||
					      ((TClusterIds == RandomNumberGenerator::Id) || ...)) {
					if (id == RandomNumberGenerator::Id)
						return ClusterMetadataProvider<
							DataModel::AcceptedCommandEntry,
							RandomNumberGenerator::Id>::EntryFor(command);
				}
				if constexpr (sizeof...(TClusterIds) == 0 ||
					      ((TClusterIds == ScenesManagement::Id) || ...)) {
					if (id == ScenesManagement::Id)
						return ClusterMetadataProvider<DataModel::AcceptedCommandEntry,
									       ScenesManagement::Id>::EntryFor(command);
				}
