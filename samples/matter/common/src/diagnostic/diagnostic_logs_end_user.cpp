...
CHIP_ERROR DiagnosticLogsEndUser::Init()
{
	...
	ret = retention_is_valid(kEndUserDataMem);
	...
	ret = retention_read(kEndUserDataMem, 0, reinterpret_cast<uint8_t *>(&mCurrentSize), sizeof(mCurrentSize));
...
CHIP_ERROR DiagnosticLogsEndUser::Clear(){ return System::MapErrorZephyr(retention_clear(kEndUserDataMem)); }
...
CHIP_ERROR DiagnosticLogsEndUser::PushLog(const void *data, size_t size)
{
	...
	if (mCapacity - mCurrentSize <= size) {
		mCurrentSize -= size;

		/* Detect buffer wrapping around. */
		mDataBegin = mDataBegin + size > mCapacity ? mDataBegin + size - mCapacity : mDataBegin + size;
