...
void DiagnosticLogProvider::DeleteIntentImplementation(IntentEnum intent, IntentData &intentData)
{
	if (!intentData.mIntentImpl) {
		return;
	}
	...
	case chip::app::Clusters::DiagnosticLogs::IntentEnum::kCrashLogs:
#ifdef CONFIG_NCS_SAMPLE_MATTER_DIAGNOSTIC_LOGS_CRASH_LOGS
		Platform::Delete(reinterpret_cast<CrashData *>(intentData.mIntentImpl));
		break;
#endif /* CONFIG_NCS_SAMPLE_MATTER_DIAGNOSTIC_LOGS_CRASH_LOGS */
	case chip::app::Clusters::DiagnosticLogs::IntentEnum::kNetworkDiag:
	case chip::app::Clusters::DiagnosticLogs::IntentEnum::kEndUserSupport:
...
CHIP_ERROR DiagnosticLogProvider::EndLogCollection(LogSessionHandle sessionHandle)
{
	...
	IntentData intentData = mIntentMap[sessionHandle];

	/* Do the specific action for the removing intent */
	switch (intentData.mIntent) {
	case IntentEnum::kEndUserSupport:
...
#ifdef CONFIG_NCS_SAMPLE_MATTER_DIAGNOSTIC_LOGS_CRASH_LOGS
		if (intentData.mIntentImpl) {
			err = reinterpret_cast<CrashData *>(intentData.mIntentImpl)->FinishCrashLogs();
		}
		break;
#else
		err = CHIP_ERROR_NOT_IMPLEMENTED;
	...
	DeleteIntentImplementation(mIntentMap[sessionHandle].mIntent, intentData);
					     ...
					     bool &outIsEndOfLog)
{
	...
	case IntentEnum::kCrashLogs:
#ifdef CONFIG_NCS_SAMPLE_MATTER_DIAGNOSTIC_LOGS_CRASH_LOGS
		err = reinterpret_cast<CrashData *>(mIntentMap[sessionHandle].mIntentImpl)
			      ->GetCrashLogs(outBuffer, outIsEndOfLog);
		break;
#else
		return CHIP_ERROR_NOT_IMPLEMENTED;
#endif /* CONFIG_NCS_SAMPLE_MATTER_DIAGNOSTIC_LOGS_CRASH_LOGS */
...
size_t DiagnosticLogProvider::GetSizeForIntent(IntentEnum intent)
{
	...
	case IntentEnum::kCrashLogs: {
		...
		logSize = crashData->GetCrashLogsSize();
		break;
	}
#endif /* CONFIG_NCS_SAMPLE_MATTER_DIAGNOSTIC_LOGS_CRASH_LOGS */
	default:
