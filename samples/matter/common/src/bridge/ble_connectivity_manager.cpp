...
BLEConnectivityManager BLEConnectivityManager::sBLEConnectivityManager;
...
void BLEConnectivityManager::FilterMatch(struct bt_scan_device_info *device_info,
					 struct bt_scan_filter_match *filter_match, bool connectable)
{
	/* Limit the number of devices that can be scanned. */
	if (GetBLEConnectivityManager().mScannedDevicesCounter >= kMaxScannedDevices) {
		return;
	}

	/* Verify the device address to make sure that the scan result will be handled only once for every device. */
	for (int i = 0; i < GetBLEConnectivityManager().mScannedDevicesCounter; i++) {
		if (memcmp(device_info->recv_info->addr, &GetBLEConnectivityManager().mScannedDevices[i].mAddr,
			   sizeof(GetBLEConnectivityManager().mScannedDevices[i].mAddr)) == 0) {
			return;
		}
	}

	memcpy(&GetBLEConnectivityManager().mScannedDevices[GetBLEConnectivityManager().mScannedDevicesCounter].mAddr,
	       device_info->recv_info->addr,
	       sizeof(GetBLEConnectivityManager()
			      .mScannedDevices[GetBLEConnectivityManager().mScannedDevicesCounter]
			      .mAddr));
	memcpy(&GetBLEConnectivityManager()
			.mScannedDevices[GetBLEConnectivityManager().mScannedDevicesCounter]
			.mConnParam,
	...
	/* TODO: obtain more information from advertising data to help user distinguish the devices (e.g. name). */

	GetBLEConnectivityManager().mScannedDevicesCounter++;
}

void BLEConnectivityManager::ConnectionHandler(struct bt_conn *conn, uint8_t conn_err)
{
	...
	for (int i = 0; i < GetBLEConnectivityManager().mCreatedDevicesCounter; i++) {
		if (memcmp(&GetBLEConnectivityManager().mCreatedDevices[i].mAddr, addr,
			   sizeof(GetBLEConnectivityManager().mCreatedDevices[i].mAddr)) == 0) {
			device = &GetBLEConnectivityManager().mCreatedDevices[i];
			break;
		}
	}
...
void BLEConnectivityManager::DisconnectionHandler(struct bt_conn *conn, uint8_t reason)
{
...
void BLEConnectivityManager::DiscoveryCompletedHandler(struct bt_gatt_dm *dm, void *context)
{
...
void BLEConnectivityManager::DiscoveryNotFound(struct bt_conn *conn, void *context)
{
...
void BLEConnectivityManager::DiscoveryError(struct bt_conn *conn, int err, void *context)
{
...
CHIP_ERROR BLEConnectivityManager::Scan()
{
	if (mScanActive) {
...
{
	GetBLEConnectivityManager().StopScan();

	/* TODO: invoke callback from the upper layer instead of printing using logger */
	LOG_INF("Scan result:");
	LOG_INF("-----------------------------");
	LOG_INF("| Index |      Address      |");
	LOG_INF("-----------------------------");
	for (int i = 0; i < GetBLEConnectivityManager().mScannedDevicesCounter; i++) {
		LOG_INF("| %d     | %02x:%02x:%02x:%02x:%02x:%02x |", i,
			GetBLEConnectivityManager().mScannedDevices[i].mAddr.a.val[5],
			GetBLEConnectivityManager().mScannedDevices[i].mAddr.a.val[4],
			GetBLEConnectivityManager().mScannedDevices[i].mAddr.a.val[3],
			GetBLEConnectivityManager().mScannedDevices[i].mAddr.a.val[2],
			GetBLEConnectivityManager().mScannedDevices[i].mAddr.a.val[1],
			GetBLEConnectivityManager().mScannedDevices[i].mAddr.a.val[0]);
	}
}
...
{
	struct bt_conn_info info_in, info_local;
