...
namespace
{
template <class T> bool LoadSimpleData(PersistentStorageNode *node, T &data)
{
	T tempData;
	size_t readSize = 0;

	bool result = PersistentStorage::Instance().Load(node, &tempData, sizeof(T), readSize);
...
{
	return LoadSimpleData(&mBridgedDevicesCount, count);
}
...
bool BridgeStorageManager::LoadBridgedDeviceEndpointId(uint16_t &endpointId, uint8_t bridgedDeviceIndex)
{
	...
	return LoadSimpleData(&id, endpointId);
}
...
bool BridgeStorageManager::LoadBridgedDeviceType(uint16_t &deviceType, uint8_t bridgedDeviceIndex)
{
	...
	return LoadSimpleData(&id, deviceType);
}
...
bool BridgeStorageManager::StoreBridgedDevice(BridgedDevice *device, uint8_t index)
{
	...
	if (!StoreBridgedDeviceEndpointId(static_cast<uint16_t>(device->GetEndpointId()), index)) {
		return false;
	}
	...
	return StoreBridgedDeviceType(static_cast<uint16_t>(device->GetDeviceType()), index);
}
