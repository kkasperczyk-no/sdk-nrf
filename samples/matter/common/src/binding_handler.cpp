...
CHIP_ERROR BindingHandler::Init(DeviceChangedHandler handler)
{
	VerifyOrReturnError(mDeviceChangedHandler != nullptr, CHIP_ERROR_INVALID_ARGUMENT);
	InitInternal();
...
void BindingHandler::OnInvokeCommandSucces()
{
	...
	if (BindingHandler::Instance().mCaseSessionRecovered)
		BindingHandler::Instance().mCaseSessionRecovered = false;
}
void BindingHandler::OnInvokeCommandFailure(BindingData &bindingData, CHIP_ERROR aError)
{
	CHIP_ERROR error;

	if (aError == CHIP_ERROR_TIMEOUT && !BindingHandler::Instance().mCaseSessionRecovered) {
		...
		BindingHandler::Instance().mCaseSessionRecovered = true;

		/* Allocate new object to make sure its life time will be appropriate. */
		BindingHandler::BindingData *data = Platform::New<BindingHandler::BindingData>();
		VerifyOrReturn(data != nullptr, LOG_ERR("New object allocation failed"););
		*data = bindingData;
		...
		error = BindingManager::GetInstance().NotifyBoundClusterChanged(
			bindingData.EndpointId, bindingData.ClusterId, static_cast<void *>(data));
...
void BindingHandler::DeviceChangedCallback(const EmberBindingTableEntry &binding, OperationalDeviceProxy *deviceProxy,
					   void *context)
{
	VerifyOrReturn(context != nullptr, LOG_ERR("Invalid context for the device handler"););
	BindingHandler::Instance().mDeviceChangedHandler(binding, deviceProxy, context);
}
