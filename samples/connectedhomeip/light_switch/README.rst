...
This light switch sample demonstrates the usage of the `Connected Home over IP`_ application layer to build a basic controller device for the `chip_light_bulb_sample` sample.
...
For this sample to work, you also need the `chip_light_bulb_sample` sample programmed to another supported development kit.
...
After building this and `chip_light_bulb_sample` samples, and programming them to your development kits, test communication between both devices by performing the following steps:

.. chip_light_switch_sample_testing_start

#. |connect_kit|
   Complete this step for both kits.
#. |connect_terminal|
   Complete this step for both kits.
...
#. Pair both devices by completing the following steps:
   ...
   b. On the light bulb device, press **Button 3** to enable pairing on this device.
      ...
      .. code-block:: console
         ...
         I: Pairing with light bulb fe80::c051:830b:2a41:6cf1
