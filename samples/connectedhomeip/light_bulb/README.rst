...
.. note::
    ...
    However, it is required when testing the `chip_light_switch_sample` sample.
...
If you want to use the :ref:`test mode <chip_light_bulb_sample_test_mode>` and control the light bulb using light switch, the `chip_light_switch_sample` sample also needs to be programmed to another supported development kit.
Note that sample needs to be built with the :ref:`light switch overlay <chip_light_bulb_building_with_light_switch_overlay>` to support testing with the light switch.
...
The light bulb device programmed with this sample can be used with the light switch device programmed with the `chip_light_switch_sample` sample to create a simplified Thread network.
...
.. _chip_light_bulb_building_with_light_switch_overlay:

Building for Light switch test mode
===================================

To build the sample with configuration that allows testing with light switch sample, run the following command:

    .. code-block:: console

        west build -b board-name -- -DOVERLAY_CONFIG=ligh_switch_test.overlay
...
After building this and `chip_light_switch_sample` samples and programming them to development kits, test communication between both devices by completing the following steps:
