...
* Added:
  ...
  * Enhanced Multi-Admin that aims to simplify the smart home management from the user perspective.
    ...
    The Fabric Synchronization enables commissioning of devices from one fabric to another without require manual user actions, but only user consent.
  ...
  * The Kconfig option :kconfig:option:`CONFIG_CHIP_ICD_SIT_SLOW_POLL_LIMIT` to limit the slow polling interval value for the device while it is in the SIT mode.
    This can be used for the ICD LIT device, to limit the slow poll interval used while temporarily working in the SIT mode.
  * New device types:
...
* Updated:
  ...
  * Electric Vehicle Supply Equipment (EVSE) with support for user-defined charging preferences, like specifying the time when car will be charged.
  * Occupancy sensing cluster with features like radar, vision, and ambient sensing.
  * Intermittently Connected Devices feature that included enhancements for the Long Idle Time (LIT) and Check-In Protocol, and in result changed state of this feature from provisional to certifiable.
