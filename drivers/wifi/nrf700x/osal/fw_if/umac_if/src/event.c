					 ...
					 enum wifi_nrf_fmac_if_state if_state)
{
	...
	if (if_idx >= MAX_NUM_VIFS) {
		wifi_nrf_osal_log_err(fmac_dev_ctx->fpriv->opriv,
				      ": Invalid wdev_id recd from UMAC %d\n",
				      if_idx);
		goto out;
	}
	...
	if (status != WIFI_NRF_STATUS_SUCCESS) {
		wifi_nrf_osal_log_err(fmac_dev_ctx->fpriv->opriv,
				      ": If state change callback function failed for VIF idx = %d\n",
				      if_idx);
		goto out;
	}
out:
						    ...
						    unsigned int event_len)
{
	...
	case IMG_UMAC_EVENT_TRIGGER_SCAN_START:
		if (callbk_fns->scan_start_callbk_fn)
			...
			wifi_nrf_osal_log_err(fmac_dev_ctx->fpriv->opriv,
					      ": No callback registered for event %d\n",
					      umac_hdr->cmd_evnt);
		break;
				  ...
				  struct host_rpu_msg *rpu_msg)
{
	...
	while (host_rpu_length_left > 0) {
		...
		if (status != WIFI_NRF_STATUS_SUCCESS) {
			wifi_nrf_osal_log_err(fmac_dev_ctx->fpriv->opriv,
					      ": umac_process_data_event failed\n");
			goto out;
		}
						     ...
						     void *event)
{
	...
	if (!fmac_dev_ctx->stats_req) {
		wifi_nrf_osal_log_err(fmac_dev_ctx->fpriv->opriv,
				      ": Stats recd when req was not sent!\n");
		goto out;
	}
						  ...
						  unsigned int rpu_event_len)
{
	...
	case IMG_HOST_RPU_MSG_TYPE_UMAC:
		...
		if (status != WIFI_NRF_STATUS_SUCCESS) {
			wifi_nrf_osal_log_err(fmac_dev_ctx->fpriv->opriv,
					      ": umac_event_ctrl_process failed\n");
			goto out;
		}
		break;
