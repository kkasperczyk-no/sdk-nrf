...
static struct bt_uuid *sServiceUuid = BT_UUID_LBS;
static struct bt_uuid *sUuidLED = BT_UUID_LBS_LED;
static struct bt_uuid *sUuidButton = BT_UUID_LBS_BUTTON;
static struct bt_uuid *sUuidCcc = BT_UUID_GATT_CCC;

uint8_t BleOnOffLightDataProvider::GattNotifyCallback(struct bt_conn *conn, struct bt_gatt_subscribe_params *params,
						      const void *data, uint16_t length)
{
	BLEBridgedDevice *dev = GetBLEConnectivityManager().FindBLEBridgedDevice(conn);
	...
	provider = reinterpret_cast<BleOnOffLightDataProvider *>(dev->provider);
...
void BleOnOffLightDataProvider::Init()
{
	...
	if (err) {
		LOG_ERR("Subscribe to LBS button characteristic failed");
	}
}
...
void BleOnOffLightDataProvider::GattWriteCallback(struct bt_conn *conn, uint8_t err,
						  struct bt_gatt_write_params *params)
{
	...
	BleOnOffLightDataProvider *provider = reinterpret_cast<BleOnOffLightDataProvider *>(dev->provider);
...
int BleOnOffLightDataProvider::ParseDiscoveredData(struct bt_gatt_dm *discoveredData)
{
