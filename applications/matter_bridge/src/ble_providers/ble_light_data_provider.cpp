...
static bt_uuid *sServiceUuid = BT_UUID_LBS2;
static bt_uuid *sUuidLED = BT_UUID_LBS_LED2;
static bt_uuid *sUuidButton = BT_UUID_LBS_BUTTON2;
static bt_uuid *sUuidCcc = BT_UUID_GATT_CCC;

void BleLightDataProvider::Init()
{
	/* Do nothing in this case */
}

void BleLightDataProvider::NotifyUpdateState(chip::ClusterId clusterId, chip::AttributeId attributeId, void *data,
					   size_t dataSize)
{
...
void BleLightDataProvider::GattWriteCallback(bt_conn *conn, uint8_t err, bt_gatt_write_params *params)
{
	if (!params) {
		return;
	}
	...
	BleLightDataProvider *provider = static_cast<BleLightDataProvider *>(
...
CHIP_ERROR BleLightDataProvider::UpdateState(chip::ClusterId clusterId, chip::AttributeId attributeId, uint8_t *buffer)
{
	...
	LOG_INF("Updating state of the BleLightDataProvider, cluster ID: %u, attribute ID: %u.", clusterId, attributeId);
	...
	case Clusters::OnOff::Attributes::OnOff::Id: {
		...
		mGattWriteParams.func = BleLightDataProvider::GattWriteCallback;
...
bt_uuid *BleLightDataProvider::GetServiceUuid()
{
	return sServiceUuid;
}

int BleLightDataProvider::ParseDiscoveredData(bt_gatt_dm *discoveredData)
{
...
void BleLightDataProvider::NotifyOnOffAttributeChange(intptr_t context)
{
	BleLightDataProvider *provider = reinterpret_cast<BleLightDataProvider *>(context);
...
void BleLightDataProvider::NotifySwitchCurrentPositionAttributeChange(intptr_t context)
{
	BleLightDataProvider *provider = reinterpret_cast<BleLightDataProvider *>(context);
...
bool BleLightDataProvider::CheckSubscriptionParameters(bt_gatt_subscribe_params *params)
{
